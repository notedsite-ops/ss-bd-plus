...
#endif
#if defined(CONFIG_EMMC_BOOT)
	{ .name = "emmc", .get_dev = emmc_get_dev, },
#endif
...
void dev_print (block_dev_desc_t *dev_desc)
{
    ...
    defined(CONFIG_MTK_RAW_PARTITION)
...
#ifdef CONFIG_MTK_RAW_PARTITION
	if (test_part_mtkraw(dev_desc) == 0) {
		PRINTF ("## Valid mtkraw partition found ##\n");
	    dev_desc->part_type = PART_TEYP_MTKRAW;
		return (0);
	}
#endif
...
#endif
#ifdef CONFIG_MTK_RAW_PARTITION
case PART_TEYP_MTKRAW:
	if (get_partition_info_mtkraw(dev_desc,part,info) == 0) {
		PRINTF ("## Valid EFI partition found ##\n");
		return (0);
	}
	break;
