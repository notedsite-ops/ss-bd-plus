...
//    mtd->ecc_stats.corrected = 0;
    ...
    timeout = c_timeout;
    NFI_Wait( ((*NFIECC_DECDONE & 0xFF) != u4Tmp), timeout); // wait for all block decode done
    if( 0 == timeout ) {
        return S_TIMEOUT;
    }
...
#define SUPPORT_MICRON_29F2G08ABAEA 0
#if SUPPORT_MICRON_29F2G08ABAEA

STATUS_E  NAND_COMMON_SetFeature(
        const struct mtd_info *mtd
        ,const u32  c_timeout
)
{
    u32    timeout = c_timeout, i;
    STATUS_E  ret=S_UNKNOWN_ERR;
    *NFI_CON = 0x3;
    *NFI_CNFG = 0x3340;
    *NFI_CMD = 0xEF;
    while(((*NFI_STA & 0xF) != 0x0) && ((*NFI_STA & 0x100) != 0x0) && (timeout--));
    if (0 == timeout)
    {
        ret = S_TIMEOUT;
        goto end;
    }
    *NFI_COLADDR = (*NFI_COLADDR & 0xFFFFFF00 | 0x80);
    *NFI_ADDRNOB = 0x1;	
    while(((*NFI_STA & 0xF) != 0x0) && ((*NFI_STA & 0x100) != 0x0) && (timeout--));
    if (0 == timeout)
    {
    ...
    for (i = 0; i < 4; i++)	
    {
        if (i == 0)	
            ...
            *NFI_DATAW = 0x00;
        *NFI_CON = 0x1200;
        while((*NFI_STA & 0xF) != 0x0);	
    }
    while(((*NFI_STA & 0xF) != 0x0) && ((*NFI_STA & 0x300) == 0x0) && (timeout--));
    if (0 == timeout)
    {
        ret = S_TIMEOUT;
        goto end;
...
STATUS_E  NAND_COMMON_GetFeature(
        const struct mtd_info *mtd
        ,const u32  c_timeout
)
{
...
//    register struct nand_chip *this = mtd->priv;
    ...
    switch (command)
    {
        ...
        case NAND_CMD_RESET:
            ...
            break;
#if SUPPORT_MICRON_29F2G08ABAEA
        case NAND_CMD_SET_FEATURE:
            NAND_COMMON_SetFeature(mtd, MTD_NAND_DEFAULT_TIMEOUT);
            break;

        case NAND_CMD_GET_FEATURE:
            NAND_COMMON_GetFeature(mtd, MTD_NAND_DEFAULT_TIMEOUT);
            break;
#endif         
        default:
            printk("\ncommand code = 0x%x\n", command);
...
#define NAND_HW_OFFSET  (PHYS_OFFSET + 0x80000000)
#else
#define NAND_HW_OFFSET  (PHYS_OFFSET + 0xC0000000)
#endif
static int _data_buf_index = 0;
BOOL _fgUsingDMA = TRUE;
BOOL _fgAUTO_FMT = TRUE;
...
static void  NAND_COMMON_ReadID(struct mtd_info *mtd, unsigned command)
{
    ...
    *NFI_ADDRNOB = 1;
	...
	#if CONFIG_READID6BYTE
	p4Data = this->buffers->databuf;
    for (i = 0; i < 6; i++)
    {
        *NFI_CON = NFI_RST | FIFO_FLUSH;
        // set read by byte
        *NFI_CON = SINGLE_RD | NOB_BYTE;
      //  printk("--%#02x ", *NFI_DATAR & 0xFF);
        // wait til DATA_READ is completely issued
        while( *NFI_STA  & STATUS_DATAR );
		p4Data[i] = *NFI_DATAR;
		//printk("0x%x",p4Data[i]);
...
//         volatile u32* p4TmpSpare2 = p4TmpBuf;
    ...
    *NFIECC_DECCON = DEC_EN;
    ...
    if ( AUTO_FMT_EN & (*NFI_CNFG))
    {
        *NFIECC_FDMADDR = (u32)0x7001E600;
    }
    if (_fgUsingDMA)
    {
        *NFI_CNFG |= AHB_MODE;
//            *NFI_STRADDR = (u32)p_data32 | NAND_HW_OFFSET;
    }
...
//     volatile u32* p4TmpSpare2 = p4TmpBuf;     
    ...
    {
        //*NFI_STRADDR = (u32)p_data32|NAND_HW_OFFSET;
        *NFI_STRADDR = (u32)buf|NAND_HW_OFFSET; // the AHB start address must be sat before the *NFI_ADDRNOB for auto ecc correct address
    }
				...
				const uint8_t *buf, int page, int cached, int raw)
{
    ...
    *NFIECC_ENCCON = ENC_EN;
    ...
    {
        *NFI_CNFG |= AHB_MODE ;
        *NFI_STRADDR = ((u32)buf)|NAND_HW_OFFSET;
    }
...
int board_nand_init(struct nand_chip *nand)
{
	nand->buffers = kmalloc(sizeof(struct nand_buffers ), GFP_KERNEL);
	if(!nand->buffers)
	{
      printk("nand init allocate mem fail\n");
	  return -1;
	}
    nand->options |= NAND_USE_FLASH_BBT| NAND_NO_SUBPAGE_WRITE;
#if 0
