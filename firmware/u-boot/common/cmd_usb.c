...
int do_usb(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (strncmp(argv[1], "eht",  3) == 0){
            usb_stop();
            if ( (argc != 3)  || (0 != usb_electrical_test(argv[2])) )
            {
                printf("eht parameter error ! Usage:\n");
                printf("(1) usb eht se0nak\n");
                printf("(2) usb eht testj\n");
                printf("(3) usb eht testk\n");
                printf("(4) usb eht testpkt\n");            
            }		
            return 1;
        }
...
U_BOOT_CMD(
	usb,	5,	1,	do_usb,
	"usb     - BDP USB sub-system\n",
	"reset - reset (rescan) USB controller\n"
	"usb eht - MediaTek USB Electrical test\n"
