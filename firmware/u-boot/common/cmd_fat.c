...
//mtk40580 follow code for add a para pos to fatload ----------
int do_fat_fsload_ex (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	long size;
	unsigned long offset;
	...
	size = file_fat_read_ex (argv[4], (unsigned char *) offset, count, pos);
...
U_BOOT_CMD(
	fatloadex,	7,	0,	do_fat_fsload_ex,
	"load binary file from a dos filesystem",
...
//mtk40580 follow code for get the length of file ----------
int do_fat_get_length (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	char *filename = "/";
	int ret;
	int dev=0;
	...
	if (argc < 4) {
		printf ("usage: fatgetlength <interface> <dev[:part]> <filename>\n");
		return (0);
	}
	dev = (int)simple_strtoul (argv[2], &ep, 16);
	dev_desc=get_dev(argv[1],dev);
	...
	if (argc == 4)
		ret = do_fat_read_length(argv[3], LS_NO);
	else
		ret = file_fat_ls (filename);
	...
	sprintf(buf, "%lX", ret);
	setenv("filelength", buf);

	return 0;
}

U_BOOT_CMD(
	fatgetlength,	4,	1,	do_fat_get_length,
	"Get the file length (absolute path)",
