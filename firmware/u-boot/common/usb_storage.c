...
again:         
	...
	if(mode==1) {
		printf("\n(%d) scanning bus for storage devices...\n", retry);
	}
...
//  CR-0040: Sandisk card reader need a delay before query device.	
       ...
       printf("usb_max_devs(%d), usb_max_lun(%d)\n", usb_max_devs, usb_max_lun);
       // mod by binghong@20101115 for BDP00056871
       // If first inquiry is ok, need not to retry angin.
       if ((retry++ < 1)&&(usb_max_devs==0))
            goto again;
